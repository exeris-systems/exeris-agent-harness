...
def run_pass(argv: list[str], *, cwd: str, env: dict, stderr_path: str) -> tuple[int, str]:
    """One pass of the adapter: its exit status and what it printed on standard output.

    Standard input is closed, because a pass nobody sits at has nobody to answer it. Standard error
    goes to a file beside the run, owner-only, where a failed pass can be read afterwards.
    """
...
def loop(*, adapter: str, module, launcher: str, run_dir: str, worktree: str, environment: dict,
    ...
    while True:
        number += 1
        argv = [launcher, *module.arguments(prompt_text=prompt_text, prompt_file=prompt_file,
                                            readable=readable, resume=resume)]
        env = dict(environment)
        env.update(module.environment(prompt_file=prompt_file))
        started = time.monotonic()
        code, stdout = run_pass(argv, cwd=worktree, env=env, stderr_path=os.path.join(
            run_dir, DIRECTORY, f"round-{number}.stderr"))
...
def _next(module, run_dir: str, stdout: str, resumed: str | None, number: int, judgement: dict):
    ...
    """
    session = module.resume_id(stdout=stdout, run_dir=run_dir)
    if not session:
