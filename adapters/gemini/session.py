...
this provider's runs claim. A session log exists, but nothing here has read it, and an adapter
that assumed its shape would produce counts whose meaning nobody has checked — which is worse than
no counts, because a number in a column is indistinguishable from a measurement.

So `locate` refuses with the reason that says exactly that, and the refusal is counted. The day
this file reads that log for real is the day the reason stops being written, and it is a
